...
#define lasterr (b1ggerror.lasterr_)
#define lastrou (b1ggerror.lastrou_)

b1ggerror_t b1ggerror = {
/* Error parameters
*/
    /* lasterr */ 0,
    /* lastrou */ 0,
    ...
    FILE *fnum;
{
    ...
    b1gprerrmsg(fnum,lastrou,lasterr);
}
...
b1gldudcomp(n,na,map,tol,ld,sum,m,l,d)
    int n,na,map[];
...
b1gldubslv(n,na,map,work,l,d,b,x)
    int n,na,map[];
    double work[],l[],d[],b[],x[];
{

    b1gldubsl(n,na,map,l,b,work);
    b1gldubsd(n,na,map,d,work,work);
    b1gldubsu(n,na,map,l,work,x);
}

b1glduslv(n,na,map,tol,work1,work2,m,b,l,d,x)
    int n,na,map[];
    double tol,work1[],work2[],m[],b[],l[],d[],x[];
{

    b1gldudcomp(n,na,map,tol,work1,work2,m,l,d);
    b1gldubslv(n,na,map,work2,l,d,b,x);
}
...
b1gqrdcomp(nr,nc,nra,nca,mapr,mapc,w,qraux,jpvt)
...
b1gqrsl(nr,nc,nra,nca,mapr,mapc,k,work,w,qraux,b,x)
...
b1gqrbslv(nr,nc,nra,nca,mapr,mapc,tol,work,iwork,w,qraux,jpvt,b,x,rank)
    int nr,nc,nra,nca,mapr[],mapc[];
    double tol,work[];
    int iwork[];
    double w[],qraux[];
...
b1gqrslv(nr,nc,nra,nca,mapr,mapc,tol,jpvt,qraux,work,iwork,w,b,x,rank)
    int nr,nc,nra,nca,mapr[],mapc[];
    double tol;
    int jpvt[];
    double qraux[],work[];
...
b1glsslv(nr,nc,nra,nca,ndes,mapr,mapc,tol,dw,rw,iw,w,b,x)
    ...
    double w[],b[],x[];
{
    ...
    if (ndes  ==  0) {
        ...
        b1gqrslv(nra,nra,nra,nra,&iw[map],&iw[map],tol,&iw[jpvt],&rw[qraux],&rw[
        ...
        b1gqrslv(dsiz,dsiz,dsiz,dsiz,&iw[map],&iw[map],tol,&iw[jpvt],&rw[qraux],
...
/* Utility routines for use with b1groot. */


b1gcalcerrs(fval,nfunc,ndes,dnorm,maxderr,maxrerr,derrnorm)
    double fval[];
...
b1gadjvars(func,vars,param,nfunc,ndes,dnorm,nvar,deltas,step,rerr,tderr,rtol,
    ...
    double newvars[],newerrs[];
{
    ...
    *fcnt = *fcnt+1;
    ...
    b1gcalcerrs(newerrs,nfunc,ndes,dnorm,&maxderr,&maxrerr,&derrnorm);
...
b1gcalcjac(func,vars,param,nfunc,nvar,lock,delta,fval,ftmp,jw,fcnt,scale)
    int (*func)();
    double vars[],param[];
    int nfunc,nvar,lock[];
...
b1groot(func,vars,param,nfunc,nvar,ndesin,lock,rtol,dtol,maxeval,jw,dw,rw,iw,
      fret,fcnt,err)
    int (*func)();
    double vars[],param[];
    ...
    int *fcnt,*err;
{
    ...
    *fcnt = *fcnt+1;
    b1gcalcerrs(fret,nfunc,ndes,dnorm,&pmaxderr,&pmaxrerr,&pderrnorm);
    crude = 1e-6;
    for (ntrys = 0; ntrys < 2; ntrys++) {
        ...
        for (;;) {
            ...
            b1gcalcjac(func,vars,param,nfunc,nvar,lock,crude,fret,&rw[f1]
              ,jw,fcnt,&rw[scale]);
            for (i = 0; i < nfunc; i++) {
                rw[f1+i] = fret[i]*rw[scale+i];
            }
            b1glsslv(nfunc,nvar,nfunc,nvar,ndes,&iw[mapf],&iw[mapv],qrtol,dw,&rw
              [morerw],&iw[moreiw],jw,&rw[f1],&rw[deltav]);
            for (;;) {
                b1gadjvars(func,vars,param,nfunc,ndes,dnorm,nvar,&rw[deltav]
                  ,step,pmaxrerr,pderrnorm,rtol,fcnt,&rw[guess],&rw[f1]);
                b1gcalcerrs(&rw[f1],nfunc,ndes,dnorm,&maxderr,&maxrerr,&
                  derrnorm);
...
/* Utility routine for use with b1gfinteg and b1gvinteg.  Work is 2*neq. */


b1grk4m(func,time,st,dst0,param,step,nst,neq,work,errs,maxerr,which)
    int (*func)();
...
b1gfinteg(func,time,st,dst,param,step,neq,work,errest,status)
    int (*func)();
    ...
    int *status;
{
    ...
    ttime = *time;
    if (step > 0.) {
        b1grk4m(func,ttime,st,dst,param,step,&work[nst],neq,&work[morework],&
...
b1gvinteg(func,time,st,dst,param,dt,step,neqin,tol,work,err,which)
    int (*func)();
    ...
    int *err,*which;
{
    ...
    *err = 0;
    ...
    while (ttime  <  tfin) {
        ...
        nstp = stp;
        for (;;) {
            b1grk4m(func,ttime,st,dst,param,nstp,&work[xst],neq,&work[morework],
              &work[errs],&maxerr,&wh);
            xtime = ttime+nstp;
            if (maxerr <= tol) {
                errf = 0;
            ...
            if (stp2 < minstep) {
                ...
                b1grk4m(func,xtime,&work[xst],&work[xdst],param,stp2,&work[xst2]
...
b1gquat2dc(ie1,ie2,ie3,ie4,dircos)
...
b1gvset(sclr1,sclr2,sclr3,ovec)
...
b1gvaxpy(sclr,ivec1,ivec2,ovec)
...
*/
    ...
    stheta = sin(theta);
    ctheta = cos(theta);
    b1gvmul(stheta,ivec,isinvec);
    b1gvmul(ctheta,ivec,icosvec);
    norm = 1./norm;
    b1gvmul(norm,rvec,uvec);
    idotu = b1gvdot(ivec,uvec);
    b1gvcross(uvec,isinvec,uxisin);
    dotcos = idotu*(1.-ctheta);
    b1gvaxpy(dotcos,uvec,icosvec,ovec);
    b1gvadd(ovec,uxisin,ovec);
}

b1gprerrmsg(fnum,routine,errnum)
    FILE *fnum;
    int routine,errnum;
{
    ...
    fprintf(fnum,"\n");
    if (routine == 1) {
        fprintf(fnum,"b1ggrav:\n");
    }
    if (routine == 2) {
        fprintf(fnum,"b1gmass:\n");
    }
    if (routine == 3) {
        fprintf(fnum,"b1giner:\n");
    }
    if (routine == 4) {
        fprintf(fnum,"b1gbtj:\n");
    }
    ...
    if (routine == 7) {
        fprintf(fnum,"b1ginit:\n");
    }
    if (routine == 8) {
        fprintf(fnum,"b1gstate:\n");
    }
    if (routine == 9) {
        fprintf(fnum,"b1gpsstate:\n");
    }
    if (routine == 13) {
        fprintf(fnum,"b1gpresacc:\n");
    }
    if (routine == 14) {
        fprintf(fnum,"b1gpresvel:\n");
    }
    if (routine == 15) {
        fprintf(fnum,"b1gprespos:\n");
    }
    if (routine == 10) {
        fprintf(fnum,"b1ghinget:\n");
    }
    if (routine == 11) {
        fprintf(fnum,"b1gpointf:\n");
    }
    if (routine == 12) {
        fprintf(fnum,"b1gbodyt:\n");
    }
    if (routine == 17) {
        fprintf(fnum,"b1gderiv:\n");
    }
    if (routine == 16) {
        fprintf(fnum,"b1gresid:\n");
    }
    if (routine == 18) {
        fprintf(fnum,"b1gpseudo:\n");
    }
    if (routine == 19) {
        fprintf(fnum,"b1gmom:\n");
    }
    ...
    if (routine == 23) {
        fprintf(fnum,"b1gorient:\n");
    }
    if (routine == 24) {
        fprintf(fnum,"b1gangvel:\n");
    }
    if (routine == 25) {
        fprintf(fnum,"b1gtrans:\n");
    }
    if (routine == 26) {
        fprintf(fnum,"b1gperr:\n");
    }
    if (routine == 27) {
        fprintf(fnum,"b1gverr:\n");
    }
    if (routine == 28) {
        fprintf(fnum,"b1gpsqdot:\n");
    }
    if (routine == 29) {
        fprintf(fnum,"b1gpsudot:\n");
    }
    if (routine == 30) {
        fprintf(fnum,"b1ggetht:\n");
    }
    if (routine == 31) {
        fprintf(fnum,"b1greac:\n");
    }
    if (routine == 32) {
        fprintf(fnum,"b1gacc:\n");
    }
    if (routine == 33) {
        fprintf(fnum,"b1gangacc:\n");
    }
    if (routine == 34) {
        fprintf(fnum,"b1gmult:\n");
    }
    if (routine == 35) {
        fprintf(fnum,"b1gaerr:\n");
    }
    if (routine == 36) {
        fprintf(fnum,"b1gindx:\n");
    }
    if (routine == 37) {
        fprintf(fnum,"b1gpres:\n");
    }
    if (routine == 38) {
        fprintf(fnum,"b1gstab:\n");
    }
    if (routine == 39) {
        fprintf(fnum,"b1ggetgrav:\n");
    }
    if (routine == 40) {
        fprintf(fnum,"b1ggetmass:\n");
    }
    if (routine == 41) {
        fprintf(fnum,"b1ggetiner:\n");
    }
    if (routine == 42) {
        fprintf(fnum,"b1ggetbtj:\n");
    }
    if (routine == 43) {
        fprintf(fnum,"b1ggetitj:\n");
    }
    if (routine == 44) {
        fprintf(fnum,"b1ggetpin:\n");
    }
    if (routine == 45) {
        fprintf(fnum,"b1ggetpres:\n");
    }
    if (routine == 46) {
        fprintf(fnum,"b1ggetstab:\n");
    }
    if (routine == 47) {
        fprintf(fnum,"b1ginfo:\n");
    }
    if (routine == 48) {
        fprintf(fnum,"b1gjnt:\n");
    }
    if (routine == 49) {
        fprintf(fnum,"b1gcons:\n");
    }
    if (routine == 50) {
        fprintf(fnum,"b1gassemble:\n");
    }
    if (routine == 51) {
        fprintf(fnum,"b1ginitvel:\n");
    }
    if (routine == 52) {
        fprintf(fnum,"b1gstatic:\n");
    }
    if (routine == 53) {
        fprintf(fnum,"b1gsteady:\n");
    }
    if (routine == 54) {
        fprintf(fnum,"b1gmotion:\n");
    }
    if (routine == 55) {
        fprintf(fnum,"b1gfmotion:\n");
    }
    if (routine == 56) {
        fprintf(fnum,"b1gequivht:\n");
    }
    if (routine == 57) {
        fprintf(fnum,"b1gmassmat:\n");
    }
    if (routine == 58) {
        fprintf(fnum,"b1gfrcmat:\n");
    }
    if (routine == 59) {
        fprintf(fnum,"b1grel2cart:\n");
    }
    if (routine == 60) {
        fprintf(fnum,"b1gcomptrq:\n");
    }
    if (routine == 61) {
        fprintf(fnum,"b1gfulltrq:\n");
    }
    if (routine == 62) {
        fprintf(fnum,"b1gvrot:\n");
    }
    if (errnum == 1) {
    ...
    if (errnum == 22) {
        fprintf(fnum,"b1ginit must be called first\n");
    }
    if (errnum == 23) {
        fprintf(fnum,"b1gstate must be called first\n");
    }
    if (errnum == 24) {
        fprintf(fnum,"b1gderiv must be called first\n");
    }
    if (errnum == 25) {
