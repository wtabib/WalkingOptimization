...
#define lasterr (b1cgerror.lasterr_)
#define lastrou (b1cgerror.lastrou_)

b1cgerror_t b1cgerror = {
/* Error parameters
*/
    /* lasterr */ 0,
    /* lastrou */ 0,
    ...
    FILE *fnum;
{
    ...
    b1cprerrmsg(fnum,lastrou,lasterr);
}
...
b1cldudcomp(n,na,map,tol,ld,sum,m,l,d)
    int n,na,map[];
...
b1cldubslv(n,na,map,work,l,d,b,x)
    int n,na,map[];
    double work[],l[],d[],b[],x[];
{

    b1cldubsl(n,na,map,l,b,work);
    b1cldubsd(n,na,map,d,work,work);
    b1cldubsu(n,na,map,l,work,x);
}

b1clduslv(n,na,map,tol,work1,work2,m,b,l,d,x)
    int n,na,map[];
    double tol,work1[],work2[],m[],b[],l[],d[],x[];
{

    b1cldudcomp(n,na,map,tol,work1,work2,m,l,d);
    b1cldubslv(n,na,map,work2,l,d,b,x);
}
...
b1cqrdcomp(nr,nc,nra,nca,mapr,mapc,w,qraux,jpvt)
...
b1cqrsl(nr,nc,nra,nca,mapr,mapc,k,work,w,qraux,b,x)
...
b1cqrbslv(nr,nc,nra,nca,mapr,mapc,tol,work,iwork,w,qraux,jpvt,b,x,rank)
    int nr,nc,nra,nca,mapr[],mapc[];
    double tol,work[];
    int iwork[];
    double w[],qraux[];
...
b1cqrslv(nr,nc,nra,nca,mapr,mapc,tol,jpvt,qraux,work,iwork,w,b,x,rank)
    int nr,nc,nra,nca,mapr[],mapc[];
    double tol;
    int jpvt[];
    double qraux[],work[];
...
b1clsslv(nr,nc,nra,nca,ndes,mapr,mapc,tol,dw,rw,iw,w,b,x)
    ...
    double w[],b[],x[];
{
    ...
    if (ndes  ==  0) {
        ...
        b1cqrslv(nra,nra,nra,nra,&iw[map],&iw[map],tol,&iw[jpvt],&rw[qraux],&rw[
        ...
        b1cqrslv(dsiz,dsiz,dsiz,dsiz,&iw[map],&iw[map],tol,&iw[jpvt],&rw[qraux],
...
/* Utility routines for use with b1croot. */


b1ccalcerrs(fval,nfunc,ndes,dnorm,maxderr,maxrerr,derrnorm)
    double fval[];
...
b1cadjvars(func,vars,param,nfunc,ndes,dnorm,nvar,deltas,step,rerr,tderr,rtol,
    ...
    double newvars[],newerrs[];
{
    ...
    *fcnt = *fcnt+1;
    ...
    b1ccalcerrs(newerrs,nfunc,ndes,dnorm,&maxderr,&maxrerr,&derrnorm);
...
b1ccalcjac(func,vars,param,nfunc,nvar,lock,delta,fval,ftmp,jw,fcnt,scale)
    int (*func)();
    double vars[],param[];
    int nfunc,nvar,lock[];
...
b1croot(func,vars,param,nfunc,nvar,ndesin,lock,rtol,dtol,maxeval,jw,dw,rw,iw,
      fret,fcnt,err)
    int (*func)();
    double vars[],param[];
    ...
    int *fcnt,*err;
{
    ...
    *fcnt = *fcnt+1;
    b1ccalcerrs(fret,nfunc,ndes,dnorm,&pmaxderr,&pmaxrerr,&pderrnorm);
    crude = 1e-6;
    for (ntrys = 0; ntrys < 2; ntrys++) {
        ...
        for (;;) {
            ...
            b1ccalcjac(func,vars,param,nfunc,nvar,lock,crude,fret,&rw[f1]
              ,jw,fcnt,&rw[scale]);
            for (i = 0; i < nfunc; i++) {
                rw[f1+i] = fret[i]*rw[scale+i];
            }
            b1clsslv(nfunc,nvar,nfunc,nvar,ndes,&iw[mapf],&iw[mapv],qrtol,dw,&rw
              [morerw],&iw[moreiw],jw,&rw[f1],&rw[deltav]);
            for (;;) {
                b1cadjvars(func,vars,param,nfunc,ndes,dnorm,nvar,&rw[deltav]
                  ,step,pmaxrerr,pderrnorm,rtol,fcnt,&rw[guess],&rw[f1]);
                b1ccalcerrs(&rw[f1],nfunc,ndes,dnorm,&maxderr,&maxrerr,&
                  derrnorm);
...
/* Utility routine for use with b1cfinteg and b1cvinteg.  Work is 2*neq. */


b1crk4m(func,time,st,dst0,param,step,nst,neq,work,errs,maxerr,which)
    int (*func)();
...
b1cfinteg(func,time,st,dst,param,step,neq,work,errest,status)
    int (*func)();
    ...
    int *status;
{
    ...
    ttime = *time;
    if (step > 0.) {
        b1crk4m(func,ttime,st,dst,param,step,&work[nst],neq,&work[morework],&
...
b1cvinteg(func,time,st,dst,param,dt,step,neqin,tol,work,err,which)
    int (*func)();
    ...
    int *err,*which;
{
    ...
    *err = 0;
    ...
    while (ttime  <  tfin) {
        ...
        nstp = stp;
        for (;;) {
            b1crk4m(func,ttime,st,dst,param,nstp,&work[xst],neq,&work[morework],
              &work[errs],&maxerr,&wh);
            xtime = ttime+nstp;
            if (maxerr <= tol) {
                errf = 0;
            ...
            if (stp2 < minstep) {
                ...
                b1crk4m(func,xtime,&work[xst],&work[xdst],param,stp2,&work[xst2]
...
b1cquat2dc(ie1,ie2,ie3,ie4,dircos)
...
b1cvset(sclr1,sclr2,sclr3,ovec)
...
b1cvaxpy(sclr,ivec1,ivec2,ovec)
...
*/
    ...
    stheta = sin(theta);
    ctheta = cos(theta);
    b1cvmul(stheta,ivec,isinvec);
    b1cvmul(ctheta,ivec,icosvec);
    norm = 1./norm;
    b1cvmul(norm,rvec,uvec);
    idotu = b1cvdot(ivec,uvec);
    b1cvcross(uvec,isinvec,uxisin);
    dotcos = idotu*(1.-ctheta);
    b1cvaxpy(dotcos,uvec,icosvec,ovec);
    b1cvadd(ovec,uxisin,ovec);
}

b1cprerrmsg(fnum,routine,errnum)
    FILE *fnum;
    int routine,errnum;
{
    ...
    fprintf(fnum,"\n");
    if (routine == 1) {
        fprintf(fnum,"b1cgrav:\n");
    }
    if (routine == 2) {
        fprintf(fnum,"b1cmass:\n");
    }
    if (routine == 3) {
        fprintf(fnum,"b1ciner:\n");
    }
    if (routine == 4) {
        fprintf(fnum,"b1cbtj:\n");
    }
    ...
    if (routine == 7) {
        fprintf(fnum,"b1cinit:\n");
    }
    if (routine == 8) {
        fprintf(fnum,"b1cstate:\n");
    }
    if (routine == 9) {
        fprintf(fnum,"b1cpsstate:\n");
    }
    if (routine == 13) {
        fprintf(fnum,"b1cpresacc:\n");
    }
    if (routine == 14) {
        fprintf(fnum,"b1cpresvel:\n");
    }
    if (routine == 15) {
        fprintf(fnum,"b1cprespos:\n");
    }
    if (routine == 10) {
        fprintf(fnum,"b1chinget:\n");
    }
    if (routine == 11) {
        fprintf(fnum,"b1cpointf:\n");
    }
    if (routine == 12) {
        fprintf(fnum,"b1cbodyt:\n");
    }
    if (routine == 17) {
        fprintf(fnum,"b1cderiv:\n");
    }
    if (routine == 16) {
        fprintf(fnum,"b1cresid:\n");
    }
    if (routine == 18) {
        fprintf(fnum,"b1cpseudo:\n");
    }
    if (routine == 19) {
        fprintf(fnum,"b1cmom:\n");
    }
    ...
    if (routine == 23) {
        fprintf(fnum,"b1corient:\n");
    }
    if (routine == 24) {
        fprintf(fnum,"b1cangvel:\n");
    }
    if (routine == 25) {
        fprintf(fnum,"b1ctrans:\n");
    }
    if (routine == 26) {
        fprintf(fnum,"b1cperr:\n");
    }
    if (routine == 27) {
        fprintf(fnum,"b1cverr:\n");
    }
    if (routine == 28) {
        fprintf(fnum,"b1cpsqdot:\n");
    }
    if (routine == 29) {
        fprintf(fnum,"b1cpsudot:\n");
    }
    if (routine == 30) {
        fprintf(fnum,"b1cgetht:\n");
    }
    if (routine == 31) {
        fprintf(fnum,"b1creac:\n");
    }
    if (routine == 32) {
        fprintf(fnum,"b1cacc:\n");
    }
    if (routine == 33) {
        fprintf(fnum,"b1cangacc:\n");
    }
    if (routine == 34) {
        fprintf(fnum,"b1cmult:\n");
    }
    if (routine == 35) {
        fprintf(fnum,"b1caerr:\n");
    }
    if (routine == 36) {
        fprintf(fnum,"b1cindx:\n");
    }
    if (routine == 37) {
        fprintf(fnum,"b1cpres:\n");
    }
    if (routine == 38) {
        fprintf(fnum,"b1cstab:\n");
    }
    if (routine == 39) {
        fprintf(fnum,"b1cgetgrav:\n");
    }
    if (routine == 40) {
        fprintf(fnum,"b1cgetmass:\n");
    }
    if (routine == 41) {
        fprintf(fnum,"b1cgetiner:\n");
    }
    if (routine == 42) {
        fprintf(fnum,"b1cgetbtj:\n");
    }
    if (routine == 43) {
        fprintf(fnum,"b1cgetitj:\n");
    }
    if (routine == 44) {
        fprintf(fnum,"b1cgetpin:\n");
    }
    if (routine == 45) {
        fprintf(fnum,"b1cgetpres:\n");
    }
    if (routine == 46) {
        fprintf(fnum,"b1cgetstab:\n");
    }
    if (routine == 47) {
        fprintf(fnum,"b1cinfo:\n");
    }
    if (routine == 48) {
        fprintf(fnum,"b1cjnt:\n");
    }
    if (routine == 49) {
        fprintf(fnum,"b1ccons:\n");
    }
    if (routine == 50) {
        fprintf(fnum,"b1cassemble:\n");
    }
    if (routine == 51) {
        fprintf(fnum,"b1cinitvel:\n");
    }
    if (routine == 52) {
        fprintf(fnum,"b1cstatic:\n");
    }
    if (routine == 53) {
        fprintf(fnum,"b1csteady:\n");
    }
    if (routine == 54) {
        fprintf(fnum,"b1cmotion:\n");
    }
    if (routine == 55) {
        fprintf(fnum,"b1cfmotion:\n");
    }
    if (routine == 56) {
        fprintf(fnum,"b1cequivht:\n");
    }
    if (routine == 57) {
        fprintf(fnum,"b1cmassmat:\n");
    }
    if (routine == 58) {
        fprintf(fnum,"b1cfrcmat:\n");
    }
    if (routine == 59) {
        fprintf(fnum,"b1crel2cart:\n");
    }
    if (routine == 60) {
        fprintf(fnum,"b1ccomptrq:\n");
    }
    if (routine == 61) {
        fprintf(fnum,"b1cfulltrq:\n");
    }
    if (routine == 62) {
        fprintf(fnum,"b1cvrot:\n");
    }
    if (errnum == 1) {
    ...
    if (errnum == 22) {
        fprintf(fnum,"b1cinit must be called first\n");
    }
    if (errnum == 23) {
        fprintf(fnum,"b1cstate must be called first\n");
    }
    if (errnum == 24) {
        fprintf(fnum,"b1cderiv must be called first\n");
    }
    if (errnum == 25) {
